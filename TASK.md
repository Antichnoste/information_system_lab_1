...
private Car car; //Поле может быть null (уточнение преподавателя: отсутствие автомобиля)
...
private Float y; //Значение поля должно быть больше -629, Поле не может быть null
}
public class Car {
private String name; //Поле может быть null
private boolean cool;
private String color; //Цвет автомобиля (добавление разрешено преподавателем; String — предложенный тип)
}
public enum Mood {
SORROW,
LONGING,
...
В системе должен быть реализован отдельный пользовательский интерфейс для выполнения специальных операций над объектами:
    ...
    Удалить один (любой) объект, значение поля weaponType которого эквивалентно заданному.
    Вернуть один (любой) объект, значение поля minutesOfWaiting которого является минимальным.
    Вернуть массив объектов, значение поля soundtrackName которых содержит заданную подстроку.
