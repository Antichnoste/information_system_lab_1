...
export default function DeleteDialog({kind,entity,onClose}:{kind:Kind;entity:Entity;onClose:()=>void}){
 ...
 async function remove(){setBusy(true);setError(undefined);try{await api(`/${kind}/${entity.id}${replacement?'?replacementId='+replacement:''}`,'DELETE');await refresh();onClose()}catch(e){setError(e)}finally{setBusy(false)}}
