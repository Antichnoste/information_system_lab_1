...
import {api,queryClient,refresh,setCsrf} from './api';
...
function Login(){
 ...
 async function submit(e:FormEvent){e.preventDefault();setBusy(true);setError(undefined);try{const session=await api<Session>('/auth/login','POST',{username,password});setCsrf(session.csrfToken);queryClient.clear();queryClient.setQueryData(['session'],session)}catch(e){setError(e)}finally{setBusy(false)}}
...
export default function App(){
 ...
 const [online,setOnline]=useState(false);const [error,setError]=useState<unknown>();
 const [modal,setModal]=useState<{kind:Kind;entity?:Entity;mode:'view'|'edit'|'delete'}|null>(null);
 const location=useLocation();
 useEffect(()=>{
  if(!session.data){setCsrf('');setModal(null);return}
  setCsrf(session.data.csrfToken);
  const stream=new EventSource('/api/events');
  stream.addEventListener('ready',()=>{setOnline(true);void refresh()});
  stream.addEventListener('changed',()=>void refresh());
  stream.onerror=()=>{setOnline(false);void queryClient.invalidateQueries({queryKey:['session']})};
  return()=>stream.close();
 },[session.data]);
 ...
 async function logout(){try{await api('/auth/logout','POST');setCsrf('');queryClient.clear();queryClient.setQueryData(['session'],null)}catch(e){setError(e)}}
